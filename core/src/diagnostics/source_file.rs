...
use super::{FileLineCol, FileName, Position, Selection, SourceId, SourceSpan};
...
impl SourceFile {
    ...
    pub fn line_column_to_span(
        ...
        let offset = self
            .content
            .line_column_to_offset(line.into(), column.into())?;
...
impl SourceFileRef {
    ...
    pub fn new(file: Arc<SourceFile>, span: impl Into<Range<u32>>) -> Self {
        ...
        let span = SourceSpan::new(file.id(), Range::new(span.start, end));
        Self { file, span }
    }
...
impl<'a> miette::SpanContents<'a> for ScopedSourceFileRef<'a> {
    ...
    fn column(&self) -> usize {
        ...
        let span = SourceSpan::new(self.file.id(), Range::new(start, end));
...
impl SourceContent {
    ...
    pub fn source_range(&self) -> Range<ByteIndex> {
        Range::new(ByteIndex::new(0), ByteIndex::new(self.content.len() as u32))
    }
    ...
    pub fn source_slice(&self, span: impl Into<Range<usize>>) -> Option<&str> {
        let span = span.into();
        self.as_str().get(span.into_range())
    }
    ...
    pub fn line_range(&self, line_index: LineIndex) -> Option<Range<ByteIndex>> {
        ...
        match self.line_start(line_index + 1) {
            Some(line_end) => Some(Range::new(line_start, line_end)),
            None => Some(Range::new(line_start, ByteIndex(self.content.len() as u32))),
        }
    }
    ...
    pub fn update(
        ...
        match range {
            Some(range) => {
                ...
                assert!(
                    start <= end,
                    "start of range must be less than end, got {start}..{end}",
                );
                ...
                match deletion {
                    Deletion::Empty => {
                        self.line_starts
                            .splice(splice_start..splice_start, added_line_starts);
                    }
                    Deletion::Inclusive(end_idx) => {
                        self.line_starts
                            .splice(splice_start..=end_idx, added_line_starts);
                    }
                }
...
";
        ...
        assert_eq!(
            content
                .byte_slice(
                    content
                        .line_range(content.last_line_index())
                        .expect("invalid line")
                )
                .expect("invalid byte range"),
            "".as_bytes()
...
";
        ...
        content
            .update(
                FRAGMENT.to_string(),
                Some(Selection::from(LineIndex(4)..LineIndex(5))),
                1,
            )
